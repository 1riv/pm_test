...
public class PhoneBook2 {
    ...
    private Contact2[] contacts;
    
    // 주어진 크기만큼의 연락처 배열을 초기화
    public PhoneBook2(int size) {
        this.contacts = new Contact2[size];
    }
    ...
    public void addContact(int index) {
    	for (int i = 0; i < index; i++) {
            ...
            contacts[i] = new Contact2(name, phoneNumber);
    	}
    	// 입력과 배열에 저장이 끝나면 출력
    ...
    public void searchByName() {
    	while (true) {
            ...
            if (name.equalsIgnoreCase("stop") || name.equals("중단") || name.equals("그만")) {
                ...
                for (Contact2 refcontact : contacts) {
    ...
    public static void main(String[] args) {
        ...
        // addContact 메서드를 이용해 연락처 추가
        phonebook.addContact(headCount);        
        PhoneBook2 phonebook = new PhoneBook2(headCount);
        // 이름으로 연락처를 검색하는 무한 루프
        phonebook.searchByName(); // searchByName 메서드를 이용해 번호 찾기
    }
}
